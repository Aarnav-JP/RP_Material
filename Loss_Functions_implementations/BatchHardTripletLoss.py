...
sys.stdout = open('outputs/' + data + 'BatchHardTripletLoss'+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
class BatchHardTripletLoss(nn.Module):
    ...
    # Accumulate loss inside the SentenceTransformer model
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.savefig(f'plots/{data}_BatchHardTripletLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"BatchHardTripletLoss_{model_name}_{data}"))
...
custom_loss_fn = BatchHardTripletLoss(
    model=model,
...
with open(f"metrics/{data}_BatchHardTripletLoss_val.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_BatchHardTripletLoss_test.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_BatchHardTripletLoss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_BatchHardTripletLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_BatchHardTripletLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
plt.plot(epochs, epoch_losses, linestyle='-', color='b')
plt.title(' BatchHardTripletLoss Loss vs. Epoch')
plt.xlabel('Epoch')
plt.ylabel('Training Loss')
plt.xticks(epochs)  # Ensure x-axis has ticks for each epoch
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_BatchHardTripletLoss_losses.png", format='png', bbox_inches='tight', dpi=300)
