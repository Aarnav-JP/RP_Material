...
sys.stdout = open('outputs/' + data + '_singlecosloss_'+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
class CustomCosineSimilarityLoss(nn.Module):
    def __init__(self, model: ModifiedSentenceTransformer,
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor) -> Tensor:
        ...
        # batch_loss = self.loss_fct(output, labels.float().view(-1))
        # # print(batch_loss)
        # model.current_epoch_loss = model.current_epoch_loss + batch_loss.item()
...
def modified_lpa(train_emb, test_emb, Ytrain, n_yval, iter, sigma):
    ...
    return F[val_mask].real, F[test_mask].real
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            cmap = plt_.colormaps['tab20']  # Use the tab20 colormap for distinct colors
            ...
            # plt.legend()
            plt_.axis([-40, 40, -40, 40])
            plt_.title('TSNE Visualization of Train Embeddings-single_cos-loss')
            plt_.savefig(f'plots/{data}_single_cosine_loss_TSNE_train' + str(self.epoch) + '.png')
...
save_path = "models/"  
custom_loss_fn = CustomCosineSimilarityLoss(model=model)
...
def plot_metrics(metric_name, val_metrics, test_metrics, title,ep,save_plot=None):
    ...
    plt_.plot(epochs, val_metrics, label=f"Validation {metric_name}", marker='o')
    plt_.plot(epochs, test_metrics, label=f"Test {metric_name}", marker='o')
    plt_.xlabel("Epochs")
    plt_.ylabel(metric_name)
    plt_.title(title)
    plt_.legend()
    plt_.show()
    if save_plot:
        plt_.savefig(save_plot, format='png', bbox_inches='tight', dpi=300)
...
# Plot the graph
plt_.figure(figsize=(8, 6))
plt_.plot(epochs, epoch_losses, linestyle='-', color='b')
plt_.title(' single_cos-sim-loss-Training Loss vs. Epoch')
plt_.xlabel('Epoch')
plt_.ylabel('Training Loss')
plt_.xticks(epochs)  # Ensure x-axis has ticks for each epoch
plt_.tight_layout()
plt_.savefig("plots/"+data+"_"+model_name+"_single_cos_losses.png", format='png', bbox_inches='tight', dpi=300)