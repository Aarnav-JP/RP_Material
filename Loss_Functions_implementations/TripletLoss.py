...
sys.stdout = open('outputs/' + data + ' TripleLoss '+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
def make_training_examples(X, y):
    train_examples = []
    # The loss function expects three embeddings per example.
    # so made necessary changes in 
    for i in range(len(X)):
        # Find a positive sample with the same label
        ...
        # Create triplet
        train_examples.append(InputExample(texts=[X[i], X[pos_index], X[neg_index]]))
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.title('TSNE Visualization of Train Embeddings-TripleLoss-loss')
            plt.savefig(f'plots/{data}_TripleLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"TripleLoss_{model_name}_{data}"))
...
# Prepare DataLoader
train_examples = make_training_examples(X_train, y_train)
train_dataloader = DataLoader(train_examples, shuffle=True, batch_size=batch_size,
                              collate_fn=lambda x: tuple(zip(*[(ex.texts) for ex in x])))

# train_dataloader = DataLoader(, shuffle=True, batch_size=batch_size)
...
custom_loss_fn = TripletLoss(model=model)  # Replace original loss initialization
...
with open(f"metrics/{data}_TripleLoss_val.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_TripleLoss_test.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_TripleLoss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_TripleLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_TripleLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
...
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_TripleLoss_losses.png", format='png', bbox_inches='tight', dpi=300)
