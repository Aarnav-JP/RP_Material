...
sys.stdout = open('outputs/' + data + ' SoftmaxLoss '+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
class SoftmaxLoss(nn.Module):
    ...
    def forward(
        ...
        # print(f"Length of sentence_features: {len(sentence_features)}")
        # print(f"First sentence_feature keys: {sentence_features[0].keys()}")

        reps = [self.model(sentence_feature)["sentence_embedding"] for sentence_feature in sentence_features]
        
        if len(reps) != 2:  
            raise ValueError(f"Expected 2 sentence embeddings, got {len(reps)}. Check if dataset provides sentence pairs.")
...
def make_training_examples(X, y):
    train_examples = []
    for i in range(len(X) - 1):  # Use (i, i+1) pairs
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.title('TSNE Visualization of Train Embeddings-Softmax-loss')
            plt.savefig(f'plots/{data}_SoftmaxLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"SoftmaxLoss_{model_name}_{data}"))
...
custom_loss_fn = SoftmaxLoss(
    model=model,
    sentence_embedding_dimension=sentence_embedding_dimension,
    num_labels=num_labels
)
  # Replace original loss initialization
...
with open(f"metrics/{data}_SoftmaxLossloss_val.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_SoftmaxLoss_test.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_SoftmaxLoss_loss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_SoftmaxLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_SoftmaxLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
plt.plot(epochs, epoch_losses, linestyle='-', color='b')
plt.title(' SoftMax Loss vs. Epoch')
plt.xlabel('Epoch')
plt.ylabel('Training Loss')
plt.xticks(epochs)  # Ensure x-axis has ticks for each epoch
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_SoftmaxLoss.png", format='png', bbox_inches='tight', dpi=300)
