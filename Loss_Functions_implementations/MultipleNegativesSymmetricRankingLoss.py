...
sys.stdout = open('outputs/' + data + ' MultipleNegativesSymmetricRankingLoss '+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
class MultipleNegativesSymmetricRankingLoss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[dict[str, Tensor]], labels: Tensor) -> Tensor:
        ...
        loss = (forward_loss + backward_loss) / 2  # Final loss

        # Update self.current_epoch_loss inside ModifiedSentenceTransformer
        if hasattr(self.model, "current_epoch_loss"):
            self.model.current_epoch_loss += loss.item()  # <-- Add this line
...
def make_training_examples(X, y):
    train_examples = []
    # made necessary changes in this because this loss function requires 2 examples at a time and 
    # also has to handle last example as well thats why len(X)-1
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.savefig(f'plots/{data}_MultipleNegativesSymmetricRankingLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"MultipleNegativesSymmetricRankingLoss_{model_name}_{data}"))
...
custom_loss_fn = MultipleNegativesSymmetricRankingLoss(model=model)  # Replace original loss initialization
...
with open(f"metrics/{data}_MultipleNegativesSymmetricRankingLoss_val.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_MultipleNegativesSymmetricRankingLoss_test.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_MultipleNegativesSymmetricRankingLoss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_MultipleNegativesSymmetricRankingLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_MultipleNegativesSymmetricRankingLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
plt.plot(epochs, epoch_losses, linestyle='-', color='b')
plt.title(' MultipleNegativesSymmetricRankingLoss vs. Epoch')
plt.xlabel('Epoch')
plt.ylabel('Training Loss')
plt.xticks(epochs)  # Ensure x-axis has ticks for each epoch
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_MultipleNegativesSymmetricRankingLoss_losses.png", format='png', bbox_inches='tight', dpi=300)
