...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.title('TSNE Visualization of Train Embeddings-CachedMultipleNegativesRankingLoss-loss')
            plt.savefig(f'plots/{data}_CachedMultipleNegativesRankingLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"CachedMultipleNegativesRankingLoss_{model_name}_{data}"))
...
with open(f"metrics/{data}_CachedMultipleNegativesRankingLoss_val.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_CachedMultipleNegativesRankingLoss_test.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_CachedMultipleNegativesRankingLoss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_CachedMultipleNegativesRankingLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_CachedMultipleNegativesRankingLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
plt.plot(epochs, epoch_losses, linestyle='-', color='b')
plt.title(' CachedMultipleNegativesRankingLoss  vs. Epoch')
plt.xlabel('Epoch')
plt.ylabel('Training Loss')
plt.xticks(epochs)  # Ensure x-axis has ticks for each epoch
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_CachedMultipleNegativesRankingLoss_losses.png", format='png', bbox_inches='tight', dpi=300)
