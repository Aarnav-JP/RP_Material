...
sys.stdout = open('outputs/' + data + ' BatchAllTripletLoss '+model_name+ '_' + now.strftime("%Y-%m-%d-%H-%M-%S") + '.txt','w')
...
class BatchHardTripletLossDistanceFunction:
    @staticmethod
    def euclidean_distance(embeddings: torch.Tensor):
        x_squared = torch.sum(embeddings ** 2, dim=1, keepdim=True)
        distances = x_squared + x_squared.t() - 2.0 * torch.matmul(embeddings, embeddings.t())
        distances = torch.sqrt(torch.clamp(distances, min=1e-6))  # Avoid NaNs
        return distances

    @staticmethod
    def cosine_distance(embeddings: torch.Tensor):
        normed = embeddings / (embeddings.norm(dim=1, keepdim=True) + 1e-6)  # Avoid division by zero
...
class BatchAllTripletLoss(nn.Module):
    ...
    def forward(self, sentence_features, labels):
        # print("First 5 sentence_features:")
        # for i in range(min(5, len(sentence_features))):
        #     print(sentence_features[i])  # Print raw inputs for debugging
        
        rep = self.sentence_embedder(sentence_features[0])['sentence_embedding']
        
        if torch.isnan(rep).any():
            print("NaN detected in sentence embeddings! Debug input:", sentence_features[0])
            rep = torch.nan_to_num(rep)  # Replace NaNs with zero
        
        # print("First 5 embeddings:", rep[:5])  # Debug embeddings

        loss = self.batch_all_triplet_loss(labels, rep)

        if torch.isnan(loss).any():
            print("NaN detected in loss computation!")

        if isinstance(self.sentence_embedder, ModifiedSentenceTransformer):
    ...
    def get_triplet_mask(self, labels):
        ...
        i_not_j = ~torch.eye(labels.shape[0], dtype=torch.bool, device=labels.device)
        anchor_positive_mask = labels_equal & i_not_j
        anchor_negative_mask = ~labels_equal
    ...
    def batch_all_triplet_loss(self, labels, embeddings):
        ...
        # print("Pairwise Distance Matrix (first 5 rows, first 5 cols):")
        # print(pairwise_dist[:5, :5])

        pairwise_dist = torch.nan_to_num(pairwise_dist)  # Replace NaNs with zero
        ...
        triplet_loss = torch.clamp(triplet_loss, min=0)  # Ensure non-negative loss

        valid_triplets = triplet_loss[triplet_loss > 1e-6]
        num_positive_triplets = valid_triplets.size(0)

        triplet_loss = triplet_loss.sum() / (num_positive_triplets + 1e-6)  # Avoid division by zero
        # print(f"Triplet Loss: {triplet_loss.item()}")
...
class Eval(evaluation.SentenceEvaluator):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        if self.epoch<11 and self.epoch%2==0:
            ...
            plt.savefig(f'plots/{data}_BatchAllTripletLoss_TSNE_train' + str(self.epoch) + '.png')
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save(os.path.join(self.save_path, f"BatchAllTripletLoss{model_name}_{data}"))
...
custom_loss_fn = BatchAllTripletLoss(model=model, margin=args.margin)  # Replace original loss initialization
...
with open(f"metrics/{data}_BatchAllTripletLoss.npy", 'wb') as g2:
   np.save(g2, val_accuracies)
   np.save(g2, val_f1_scores)

with open(f"metrics/{data}_BatchAllTripletLoss.npy", 'wb') as g3:
...
plot_metrics("Accuracy",  val_accuracies, test_accuracies, "Accuracy Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_BatchAllTripletLoss_accuracy.png")
plot_metrics("F1 Score",  val_f1_scores, test_f1_scores, "F1 Score Over Epochs",ep,save_plot="plots/"+data+"_"+model_name+"_BatchAllTripletLoss_f1scores.png")
...
np.save(f'embeddings/{data}_{model_name}_train_BatchAllTripletLoss.npy',epoch_losses)
# Plot the graph
plt.figure(figsize=(8, 6))
...
plt.tight_layout()
plt.savefig("plots/"+data+"_"+model_name+"_BatchAllTripletLoss_losses.png", format='png', bbox_inches='tight', dpi=300)
