...
class ContrastiveTensionDatasetWrapper(Dataset):
    def __init__(self, contrastive_loader):
        """
        Wrapper for ContrastiveTensionDataLoader to make it compatible with PyTorch's DataLoader.

        Args:
        - contrastive_loader: Instance of ContrastiveTensionDataLoader.
        """
...
# this code not compatible in pytorch because it gives - 'ContrastiveTensionDataLoader' object is not subscriptable
...
# # Initialize ContrastiveTensionDataLoader
# contrastive_dataloader = ContrastiveTensionDataLoader(X_train, batch_size=batch_size, pos_neg_ratio=8)

# # Convert it to an iterable DataLoader compatible with SentenceTransformers
# train_dataloader = DataLoader(contrastive_dataloader, batch_size=batch_size, shuffle=True)
